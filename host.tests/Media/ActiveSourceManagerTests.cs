...
public sealed class ActiveSourceManagerTests
{
    ...
    public async Task DiscoveryReturnsRawAumidsAsStableValues()
    {
        ...
        var discovery = await monitor.RefreshWindowsMediaSourcesAsync();
    ...
    public async Task ManagerInitializationFailureIsReportedAsUnavailable()
    {
        ...
        await using var monitor = new ActiveSourceManager(
            factory,
            new WindowsMediaSessionMatcher(),
            SourceDescriptor.WindowsMedia("Player.App"),
            NullLogger<ActiveSourceManager>.Instance);
    ...
    public async Task RapidSelectionChangeRejectsLateObservationFromPreviousAumid()
    {
        ...
        monitor.SelectWindowsMedia("Second.Player");
    ...
    private static ActiveSourceManager CreateMonitor(
        StubManager manager,
        string? selection = "Player.App")
    {
        return new ActiveSourceManager(
