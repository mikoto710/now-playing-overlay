...
internal sealed class ActiveSourceManager : ISessionSource, ISessionSourceStatus
{
    ...
    private readonly ILogger<ActiveSourceManager> _logger;
    ...
    private SourceManagerState _state;
    ...
    {
        var selection = string.IsNullOrWhiteSpace(sourceAppUserModelId)
            ? null
    ...
    public async Task<SourceDiscoveryResult> RefreshWindowsMediaSourcesAsync(
        CancellationToken cancellationToken = default)
    {
        ...
        if (initialized)
        {
            ...
            if (manager is not null)
            {
                try
                {
                    sources = RefreshBinding(manager);
                }
    ...
    public async ValueTask<SessionObservation> ReadAsync(CancellationToken cancellationToken)
    {
        while (true)
        {
            ...
            {
                ObjectDisposedException.ThrowIf(_disposed, this);
                transitionSource = _selection;
                transitionGeneration = _selectionGeneration;
            ...
            {
                ScheduleFollowUpRefresh(transitionGeneration);
            ...
            {
                throw new InvalidOperationException("Active media source monitoring failed.", backgroundError);
            }
            ...
            catch (OperationCanceledException) when (readCancellation.IsCancellationRequested)
            {
                ...
                if (HasSelectionChanged(selectionGeneration))
                {
            ...
            catch (Exception) when (HasSelectionChanged(selectionGeneration))
            {
                return CreateCurrentUnavailableObservation();
            }
            ...
            {
                MarkSessionUnavailable(session, selectionGeneration, bindingGeneration, error);
                return SessionObservation.Create(selected, PlaybackState.Unavailable);
            }
            finally
            {
            ...
            if (HasBindingChanged(session, selectionGeneration, bindingGeneration))
            {
                continue;
            }
    ...
    private IReadOnlyList<SourceDescriptor> RefreshBinding(IMediaSessionManager manager)
    {
        ...
        LogSelection(selected, selection, descriptors.Length);
        return descriptors;
    }
    ...
    private void SetFaulted(Exception error)
    {
        ...
        _logger.LogError(error, "Active source management faulted.");
    }
        ...
        long bindingGeneration)
    {
        lock (_gate)
        {
            return _selectionGeneration != selectionGeneration
                || _bindingGeneration != bindingGeneration
                || !ReferenceEquals(_boundSession, session);
        }
    }

    private bool HasSelectionChanged(long selectionGeneration)
    {
        lock (_gate)
        {
            return _selectionGeneration != selectionGeneration;
    ...
    private void ScheduleFollowUpRefresh(long selectionGeneration)
    {
        var task = FollowUpRefreshAsync(selectionGeneration, _shutdown.Token);
        lock (_gate)
        {
    ...
    private async Task FollowUpRefreshAsync(
        long selectionGeneration,
        CancellationToken cancellationToken)
    {
        ...
        lock (_gate)
        {
            if (_disposed || _selectionGeneration != selectionGeneration)
            {
                return;
            }
        }
    ...
    private static void ValidateSelection(SourceDescriptor? selection)
    {
        if (selection is not null && selection.Key.Provider != SourceProvider.WindowsMedia)
        {
            throw new ArgumentException(
                "ActiveSourceManager currently accepts only Windows Media selections.",
                nameof(selection));
        }
    }
